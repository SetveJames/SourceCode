...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public XmlBeanDefinitionReader(BeanDefinitionRegistry registry) {
		super(registry); // registry就是创建的beanFactory对象
	}
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get(); //创建一个Set集合
		...
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			...
			if (currentResources.isEmpty()) {
				this.resourcesCurrentlyBeingLoaded.remove(); //删除上面创建的Set集合
			}
		}
	}
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource) throws BeanDefinitionStoreException {
		try {
			Document doc = doLoadDocument(inputSource, resource); //把输入流封装为Document对象
