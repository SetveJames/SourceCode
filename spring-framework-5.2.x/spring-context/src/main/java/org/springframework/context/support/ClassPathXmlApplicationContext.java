...
public class ClassPathXmlApplicationContext extends AbstractXmlApplicationContext {
	...
	public ClassPathXmlApplicationContext(String[] configLocations, boolean refresh, @Nullable ApplicationContext parent) throws BeansException {
		/**
		 * 	↑	new AbstractXmlApplicationContext() //创建AbstractXmlApplicationContext对象
		 * 	↑		new AbstractRefreshableConfigApplicationContext(); //创建AbstractRefreshableConfigApplicationContext对象，并初始化成员属性
		 * 	↑			new AbstractRefreshableApplicationContext(); //创建AbstractRefreshableApplicationContext对象，并初始化成员属性
		 * 	↑				new AbstractApplicationContext(); //创建AbstractApplicationContext对象
		 * 						1.初始化成员属性
		 * 						2.创建资源模板解析器：this.resourcePatternResolver = getResourcePatternResolver(); // PathMatchingResourcePatternResolver类型
		 * 						3.设置父容器，默认为null
		 * 	↑				new DefaultResourceLoader(); //创建DefaultResourceLoader对象
		 * 						1.初始化成员属性
		 * 						2.创建ClassLoader对象classLoader;(Thread.currentThread().getContextClassLoader()获取)
		 */
		super(parent);
		 ...
		 * 			1.创建this.propertySources=new MutablePropertySources()
		 * 			2.回调子类AbstractApplicationContext#customizePropertySources(this.propertySources)方法：
		 *				① 添加系统属性信息PropertySource(调用System.getProperties())
		 * 				② 添加系统环境信息PropertySource(调用System.getenv())
		 * 2. 把xml配置文件的路径保存在 String[]数组configLocations中
		 */
		setConfigLocations(configLocations);
		if (refresh) { // refresh默认为true
			refresh();
		}
	}
