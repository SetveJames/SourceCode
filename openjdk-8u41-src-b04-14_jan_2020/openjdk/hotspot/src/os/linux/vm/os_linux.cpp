...
// Thread start routine for all newly created threads 所有新创建线程的执行入口起点
static void *java_start(Thread *thread) {
  ...
  int pid = os::current_process_id(); //获取一个进程id
  alloca(((pid ^ counter++) & 7) * 128); //分配栈内存(无需手动释放)

  ThreadLocalStorage::set_thread(thread); //保存java线程对象

  OSThread* osthread = thread->osthread(); //获取与java线程对象绑定的OS线程对象
  Monitor* sync = osthread->startThread_lock(); //启动监视器锁
  ...
  osthread->set_thread_id(os::Linux::gettid()); //给OS线程设置线程id(Linux下的线程是一个特殊的进程)
  ...
  // initialize floating point control register 初始化浮点运算控制寄存器
  os::Linux::init_thread_fpu_state();

  // handshaking with parent thread 关联父线程
  {
    MutexLockerEx ml(sync, Mutex::_no_safepoint_check_flag);

    // notify parent thread
    osthread->set_state(INITIALIZED); //设置OS线程状态为已初始化
    sync->notify_all(); //激活监视器

    // wait until os::start_thread() 等待OS线程启动
  ...
  thread->run(); //调用java的run()方法

  return 0;
}

//创建线程【参数：this指针(表示java线程对象)，线程类型，线程栈大小(默认为0)】
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  assert(thread->osthread() == NULL, "caller responsible");

  // Allocate the OSThread object 创建一个OS线程对象
  ...
  osthread->set_thread_type(thr_type); //设置OS线程类型

  // Initial state is ALLOCATED but not INITIALIZED
  osthread->set_state(ALLOCATED); //设置OS线程状态为已分配但是为初始化

  thread->set_osthread(osthread); //绑定java线程与OS线程
  ...
  // stack size
  if (os::Linux::supports_variable_stack_size()) { //判断操作系统是否支持可变栈大小
...

  {
    ...
    pthread_t tid;
	// 使用POSIX标准库创建OS线程,java_start即为线程方法的入口
