...
JVM_ENTRY(void, JVM_StartThread(JNIEnv* env, jobject jthread))
  ...
  // in Thread::start.
  {
    ...
    if (java_lang_Thread::thread(JNIHandles::resolve_non_null(jthread)) != NULL) {
      ...
      native_thread = new JavaThread(&thread_entry, sz); // 创建本地OS线程(调用thread.cpp中的JavaThread::JavaThread(ThreadFunction entry_point, size_t stack_sz))

      // At this point it may be possible that no osthread was created for the
      // JavaThread due to lack of memory(OS线程可能会由于内存不足而创建失败). 
      // Check for this situation and throw an exception if necessary. 
      // Eventually we may want to change this so
