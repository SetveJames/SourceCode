...
{ //方法开始 ↓
  ...
  initialize(); //初始化
  _jni_attach_state = _not_attaching_via_jni;
  set_entry_point(entry_point); //设置线程执行入口点,入口点就是Java代码中传入的Runnable对象的run()方法
  // Create the native thread itself.
  // %note runtime_23
  os::ThreadType thr_type = os::java_thread; //设置线程状态
  thr_type = entry_point == &compiler_thread_entry ? os::compiler_thread :
                                                     os::java_thread;
  os::create_thread(this, thr_type, stack_sz); //开始创建线程
