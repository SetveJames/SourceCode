...
// 线程状态
enum ThreadState {
  ALLOCATED,                    // Memory has been allocated but not initialized 已分配内存但未初始化
  INITIALIZED,                  // The thread has been initialized but yet started 已初始化但未启动
  RUNNABLE,                     // Has been started and is runnable, but not necessarily running 已启动为可运行状态但不一定正在运行(就绪等待CPU调度)
  MONITOR_WAIT,                 // Waiting on a contended monitor lock 等待竞争锁
  CONDVAR_WAIT,                 // Waiting on a condition variable 等待条件变量
  OBJECT_WAIT,                  // Waiting on an Object.wait() call 等待调用wait()方法
  BREAKPOINTED,                 // Suspended at breakpoint 中断挂起
  SLEEPING,                     // Thread.sleep() 线程睡眠
  ZOMBIE                        // All done, but not reclaimed yet 僵尸状态(已完成所有操作，但还没有被回收)
};
